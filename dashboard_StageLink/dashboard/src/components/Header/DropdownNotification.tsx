...
type Message = {
  id: number;
  corps: string;
  dateEnvoi: string; // ISO string format
  read?: boolean; // Assuming a 'read' field exists
};
...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:8080/notifications/unread/${userId}`);
        if (response.ok) {
  ...
  const markAsRead = async (messageId: number) => {
    try {
      const response = await fetch(`http://localhost:8080/notifications/${messageId}/mark-as-read`, {
        method: "PUT",
        headers: {
                      ...
                      notifications.map((notification) => (
                          <li key={notification.id}>
                            <button
                                ...
                                className="flex flex-col gap-2 rounded-[10px] p-2.5 hover:bg-gray-2 dark:hover:bg-dark-3"
                            >
